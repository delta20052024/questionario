document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderQuestions(stepIndex) {
        ...
        step.questions.forEach((qObj, index) => {
            ...
            if (qObj.tp === "single_choice") {
                ...
                row += `
                <div class="col-md-${qObj.w} mt-3">
                    <table class="table tsable-striped table-hover table-sm table-bfordered table-likert" data-type="likert" data-name="${qObj.n}">
                        <thead>
                            <tr>
                ...
                qObj.values.forEach(value => {
                    row += `<th scope="col" class="text-center" style="width: ${(50 / qObj.values.length)}%">${value}</th>`;
                });
                ...
                qObj.options.forEach(option => {
                    row += `<tr>
                            <th scope="row" class="text-start">${option}</th>`;
                    ...
                    qObj.values.forEach(value => {
                        ...
                        row += `
                            <td class="text-center">
                                <input type="radio" id="${radioId}" name="${radioName}" value="${value}" style="" ${isRequired} ${isChecked}>
                            </td>`;
                    });
    ...
    function showStep(stepIndex) {
        ...
        steps.forEach((step, index) => {
            renderQuestions(index); // Chiama la funzione per renderizzare le domande
        });
    ...
    function saveData(status, isComplete = false) {
        ...
        if (scorm.isAvailable()) {
            scorm.setSuspendData(JSON.stringify(questionnaireData));
            scorm.setLessonStatus(status);
            scorm.setLessonLocation(currentStep.toString());
            scorm.setSessionTime(sessionDuration);
            scorm.saveAndFinish(); // Salva sempre e termina la sessione
        } else {
...
function loadSavedData() {
    ...
    if (scorm.isAvailable()) {
        ...
        if (suspendData) {
            savedData = JSON.parse(suspendData);
        }
    } else {
